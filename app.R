  ...
  observeEvent(input$add, {
    ...
    output[[id]] <-
      if (input$tool == 'correlation') renderPlot({
        ...
        scatter_plot + geom_point() + geom_smooth(method="lm")+ ggtitle(paste0("The correlation(Pearson's r) between ",isolate(input$VARL)," and ",isolate(input$VARR)," is ",isolate(cor[4])))
